...
class DQNAgent(object):
    ...
    def step_env(self):
        ...
        # TODO (Done) store the latest observation ("frame") into the replay buffer
        # HINT: the replay buffer used here is `MemoryOptimizedReplayBuffer`
            # in dqn_utils.py
        # frame = self.env.render()  # Check if this needs more parameters
        self.replay_buffer_idx = self.replay_buffer.store_frame(self.last_obs)

        eps = self.exploration.value(self.t)

        # TODO (Done) use epsilon greedy exploration when selecting action
        ...
        if perform_random_action:
            ...
            action = self.actor.get_action(obs_hist)
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            # TODO (Done) fill in the call to the update function using the appropriate tensors
